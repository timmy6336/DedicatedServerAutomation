...
class GameDetailsPage(QWidget):
    ...
    def start_server_setup(self):
        ...
        if self.game and self.game.name.lower() == "palworld":
            ...
            self.setup_window.show()
        elif self.game and self.game.name.lower() == "rust":
            # Import here to avoid circular imports
            from setup_windows.rust_setup_window import RustServerSetupWindow
    ...
    def start_server_directly(self):
        ...
        if self.game and self.game.name.lower() == "palworld":
            ...
            try:
                # Import the Rust configuration dialog and server start function
                import sys
                parent_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
                sys.path.insert(0, os.path.join(parent_dir, "scripts"))
                sys.path.insert(0, os.path.join(parent_dir, "utils"))
                ...
                from setup_windows.rust_setup_window import RustConfigDialog
                from scripts.rust_server_startup_script import start_rust_server
                from utils.config_manager import config_manager
                
                print("Opening Rust server configuration...")
                ...
                if config_dialog.exec_() == config_dialog.Accepted:
                    ...
                    config = config_dialog.get_config()
                    print("📂 Using configured server settings")
                    print(f"🦀 Server: {config.get('server_name', 'My Rust Server')}")
                    print(f"👥 Max Players: {config.get('max_players', 100)}")
                    print(f"🗺️ World Size: {config.get('world_size', 3000)}")
                    print(f"🌐 Port: {config.get('port', 28015)}")
                    print("Starting Rust server...")
                    
                    success = start_rust_server(
                        server_name=config.get('server_name', 'My Rust Server'),
                        max_players=config.get('max_players', 100),
                        world_size=config.get('world_size', 3000),
                        seed=config.get('seed', 12345),
                        port=config.get('port', 28015),
                        pve_mode=config.get('pve_mode', False),
                        secure=config.get('secure', True)
                    )
                    if success:
                        print("✅ Rust server started successfully!")
                        print("🦀 Your survival world is now active!")
                        print(f"⚔️ Mode: {'PvE' if config.get('pve_mode', False) else 'PvP'}")
                    else:
    ...
    def uninstall_server(self):
        ...
        try:
            if self.game.name.lower() == "palworld":
            ...
            elif self.game.name.lower() == "rust":
                # Import the Rust uninstall functions
                import sys
                parent_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
                sys.path.insert(0, os.path.join(parent_dir, "scripts"))
                from scripts.rust_server_startup_script import (
                    uninstall_rust_server,
                    remove_port_forward_rule
                )
                ...
                # Uninstall server files
                if uninstall_rust_server():
                    print("✅ Rust server files removed")
                    print("🏜️ All server data and maps have been deleted")
                else:
                    print("⚠️ Failed to remove server files")
                ...
                QMessageBox.information(
                    ...
                    'Uninstallation Complete',
                    f'{self.game.name} server has been successfully uninstalled.\n\n'
                    'Server files, map data, and port forwarding rules have been removed.\n'
                    'SteamCMD has been kept for future server installations.',
                    QMessageBox.Ok
                )
    ...
    def is_rust_server_installed(self):
        """Check if Rust server files are installed"""
        # Use the same paths as the startup script
        if platform.system().lower() == 'windows':
            steamcmd_dir = os.path.expandvars(r'%USERPROFILE%\SteamCMD')
            server_dir = os.path.expandvars(r'%USERPROFILE%\Steam\steamapps\common\Rust Dedicated Server')
        else:
            steamcmd_dir = os.path.expanduser('~/SteamCMD')
            server_dir = os.path.expanduser('~/Steam/steamapps/common/Rust Dedicated Server')
