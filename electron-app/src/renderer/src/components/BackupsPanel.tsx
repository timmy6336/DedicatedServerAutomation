  ...
  return (
    <div className="h-full flex flex-col px-8 py-6 gap-4 overflow-hidden">
      ...
      <div className="flex items-center justify-between shrink-0">
        <div>
          <h2 className="text-base font-semibold text-[#e2e2ef]">Backups</h2>
          <p className="text-xs text-[#575770] mt-0.5">
            {backups.length} backup{backups.length !== 1 ? 's' : ''} · <span className="text-[#8e8ea8]">{instance.name}</span>
          </p>
        </div>
        <button
        ...
        {backups.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-14 text-center">
            <div className="w-12 h-12 rounded-xl bg-[#151521] border border-[#1c1c2e] flex items-center justify-center mb-3">
              <HardDrive size={20} className="text-[#28283f]" />
            </div>
            <p className="text-sm text-[#575770]">No backups yet</p>
            <p className="text-xs text-[#383854] mt-1 max-w-xs leading-relaxed">
          ...
          backups.map(b => (
            <div
              key={b.id}
              className="flex items-center gap-4 px-5 py-4 rounded-xl bg-[#0f0f18] border border-[#1c1c2e] hover:border-[#28283f] transition-colors"
            >
              <div className="w-9 h-9 rounded-lg bg-[#151521] border border-[#1c1c2e] flex items-center justify-center shrink-0">
                <HardDrive size={15} className="text-[#575770]" />
              </div>

              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-[#c0c0d8] truncate">{b.label}</p>
                <p className="text-xs text-[#383854] mt-0.5 font-mono tabular-nums">{formatBytes(b.sizeBytes)}</p>
              </div>
              ...
              {confirmRestore === b.id ? (
                <div className="flex items-center gap-2.5 shrink-0">
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-yellow-700/80 hover:bg-yellow-600 text-white disabled:opacity-50 transition-colors"
                  >
                    {restoring === b.id ? 'Restoring…' : 'Yes, restore'}
                  </button>
                  <button
                    onClick={() => setConfirmRestore(null)}
                    className="px-3 py-1.5 rounded-lg text-xs text-[#575770] hover:text-[#e2e2ef] hover:bg-[#151521] transition-all"
                  >
                    Cancel
                  </button>
                </div>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button
                    ...
                    className={cn(
                      ...
                      'bg-[#151521] border border-[#1c1c2e] text-[#8e8ea8]',
                      'hover:text-[#e2e2ef] hover:border-[#28283f]',
                  ...
                  <button
                    ...
                    disabled={!!restoring}
                    className="w-8 h-8 flex items-center justify-center rounded-lg text-[#383854] hover:text-red-400 hover:bg-red-950/30 disabled:opacity-40 transition-all"
                  >
                    <Trash2 size={13} />
                  </button>
                </div>
