  ...
  return (
    <div className="h-full flex flex-col px-8 py-6 gap-4">
      ...
      <div className="flex items-center justify-between shrink-0">
        <div>
          <h2 className="text-base font-semibold text-[#e8e8ff]">Server Log</h2>
        ...
        <button
          ...
          className="flex items-center gap-2 px-3.5 py-2 rounded-xl bg-[#18182a] border border-[#22223a] text-sm text-[#7070a0] hover:text-[#e8e8ff] hover:border-[#34345a] disabled:opacity-40 disabled:cursor-not-allowed transition-all"
        >
          <Trash2 size={13} />
          Clear
        </button>
      ...
      <div
        ref={logRef}
        onScroll={onScroll}
        className="flex-1 overflow-y-auto rounded-xl bg-[#080810] border border-[#22223a] p-5 font-mono text-[11px] leading-relaxed"
      >
        {lines.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center">
            <p className="text-[#404065]">
              {serverRunning
                ? 'Waiting for output…'
      ...
      {!autoScroll && lines.length > 0 && (
        <button
          ...
          className="shrink-0 flex items-center justify-center gap-2 text-xs py-2.5 rounded-xl bg-[#18182a] border border-[#22223a] text-[#7070a0] hover:text-[#e8e8ff] hover:border-[#34345a] transition-all"
        >
          <ArrowDown size={12} />
          Scroll to bottom
        </button>
